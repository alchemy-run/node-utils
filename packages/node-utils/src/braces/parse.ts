...
export function parse(input: string, options: ParseOptions = {}): Node {
  ...
  const ast: Node = { type: "root", input, nodes: [] };
  ...
  const push = (node: Node): Node | undefined => {
  ...
  while (index < length) {
    ...
    if (value === CHAR_LEFT_PARENTHESES) {
      block = push({ type: "paren", nodes: [] }) as Node;
      stack.push(block);
      push({ type: "text", value });
      continue;
    }
    ...
    if (
      ...
      const hasRightQuote =
        input
          .slice(index)
          .search(new RegExp(`[^\\\\]${leftQuote}|^${leftQuote}`)) !== -1;
  ...
  do {
    ...
    if (block.type !== "root") {
      ...
      const idx = parent.nodes.indexOf(block);
      parent.nodes.splice(idx, 1, ...block.nodes);
    }
