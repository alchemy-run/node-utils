...
describe('bash ranges - braces.compile()', () => {
  for (const arr of fixtures as Fixture[]) {
    if (typeof arr === 'string') continue;
    const [pattern, options, expected] = arr;
    if ((options as any).skip === true) continue;
    test('should compile: ' + pattern, () => {
      expect(braces.compile(pattern, options)).toEqual(expected);
    });
  }
});
