...
describe('.every()', () => {
  ...
  test('should return true if every string matches', () => {
    let fixtures = ['a/a', 'a/b', 'a/c', 'b/a', 'b/b', 'b/c'];
    expect_truthy(!mm.every(fixtures, ['z', '*/*']));
  });

  test('should return false when not all strings match', () => {
    let fixtures = ['a/a', 'a/b', 'a/c', 'b/a', 'b/b', 'b/c'];
    expect_truthy(!mm.every(fixtures, ['a/*', 'x/*']));
    expect_truthy(mm.every(fixtures, ['(a|b)/*', '*/*']));
  });

  test('should arrayify a string value', () => {
    expect_truthy(mm.every('a', '*'));
  });
});
