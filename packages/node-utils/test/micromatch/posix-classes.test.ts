...
describe('posix classes', () => {
  describe('posix bracket type conversion', () => {
    test('should create regex character classes from POSIX bracket expressions:', () => {
      expect_loose_equal(convert('foo[[:lower:]]bar'), 'foo[a-z]bar');
      expect_loose_equal(convert('foo[[:lower:][:upper:]]bar'), 'foo[a-zA-Z]bar');
      expect_loose_equal(convert('[[:alpha:]123]'), '(?=.)[a-zA-Z123]');
      expect_loose_equal(convert('[[:lower:]]'), '(?=.)[a-z]');
      expect_loose_equal(convert('[![:lower:]]'), '(?=.)[^a-z]');
      expect_loose_equal(convert('[[:digit:][:upper:][:space:]]'), '(?=.)[0-9A-Z \\t\\r\\n\\v\\f]');
      expect_loose_equal(convert('[[:xdigit:]]'), '(?=.)[A-Fa-f0-9]');
      expect_loose_equal(convert('[[:alnum:][:alpha:][:blank:][:cntrl:][:digit:][:graph:][:lower:][:print:][:punct:][:space:][:upper:][:xdigit:]]'), '(?=.)[a-zA-Z0-9a-zA-Z \\t\\x00-\\x1F\\x7F0-9\\x21-\\x7Ea-z\\x20-\\x7E \\-!"#$%&\'()\\*+,./:;<=>?@[\\]^_`{|}~ \\t\\r\\n\\v\\fA-ZA-Fa-f0-9]');
      expect_loose_equal(convert('[^[:alnum:][:alpha:][:blank:][:cntrl:][:digit:][:lower:][:space:][:upper:][:xdigit:]]'), '(?=.)[^a-zA-Z0-9a-zA-Z \\t\\x00-\\x1F\\x7F0-9a-z \\t\\r\\n\\v\\fA-ZA-Fa-f0-9]');
      expect_loose_equal(convert('[a-c[:digit:]x-z]'), '(?=.)[a-c0-9x-z]');
      if (process.platform !== 'win32') {
        expect_loose_equal(convert('[_[:alpha:]][_[:alnum:]][_[:alnum:]]*'), '(?=.)[_a-zA-Z][_a-zA-Z0-9][_a-zA-Z0-9][^/]*?', []);
      }
    });
  });

  describe('integration: posix classes with globs', () => {
    test('should work with globs', () => {
      let fixtures = ['a.b', 'a,b', 'a:b', 'a-b', 'a;b', 'a b', 'a_b'];
      expect_deepEqual(mm(fixtures, 'a[^[:alnum:]]b', { posix: true }), fixtures);
      expect_deepEqual(mm(fixtures, 'a@([^[:alnum:]])b', { posix: true }), fixtures);
      expect_deepEqual(mm(fixtures, 'a@([-.,:; _])b', { posix: true }), fixtures);

      expect_deepEqual(mm(fixtures, 'a@([^x])b', { posix: true }), ['a.b', 'a,b', 'a:b', 'a-b', 'a;b', 'a b', 'a_b']);
      expect_deepEqual(mm(fixtures, 'a+([^[:alnum:]])b', { posix: true }), fixtures);
    });
  });
  ...
  describe('.isMatch', () => {
    ...
    test('should not create an invalid posix character class:', () => {
      expect_loose_equal(convert('[:al:]'), '(?:\\[:al:\\]|[:al:])');
      expect_loose_equal(convert('[abc[:punct:][0-9]'), '(?=.)[abc\\-!"#$%&\'()\\*+,./:;<=>?@[\\]^_`{|}~\\[0-9]');
    });
  ...
  describe('.makeRe()', () => {
    test('should make a regular expression for the given pattern:', () => {
      expect_deepEqual(mm.makeRe('[[:alpha:]123]', opts), /^(?:(?=.)[a-zA-Z123])$/);
      expect_deepEqual(mm.makeRe('[![:lower:]]', opts), /^(?:(?=.)[^a-z])$/);
    });
  });
  ...
  describe('POSIX: From the test suite for the POSIX.2 (BRE) pattern matching code:', () => {
    ...
    test('should match hexidecimal digits', () => {
    ...
    test('should only match one or more punctuation characters', () => {
      expect_truthy(isMatch('?*+', '[[:punct:]]*'));
