...
describe('options.noglobstar', () => {
  test('should disable extglob support when options.noglobstar is true', () => {
    expect_truthy(isMatch('a/b/c', '**', { noglobstar: false }));
    expect_truthy(!isMatch('a/b/c', '**', { noglobstar: true }));
    expect_truthy(isMatch('a/b/c', 'a/**', { noglobstar: false }));
    expect_truthy(!isMatch('a/b/c', 'a/**', { noglobstar: true }));
  });
});
